...
public class UserController {
    ...
    public String addContact(@ModelAttribute(name = "userModel")ContactModel contactModel,
                             Model model){
        log.info("Method: addUser() -- Params: "+contactModel.toString());
        if(contactService.addContact(contactModel) != null){
            model.addAttribute("result", 1);//esto es para que se muestre un mensaje de que se agregó éxitosamente
        }else{
            model.addAttribute("result", 0);
        }
        return "redirect:/contacts/showContacts";// hay que cambiar esto
    }
